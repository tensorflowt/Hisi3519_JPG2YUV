...
HI_S32 Mat2IveImage(cv::Mat *src, IVE_DST_IMAGE_S *pstDst)
{
    ...
    IVE_IMAGE_TYPE_E ImgType = pstDst->enType;

    if(ImgType == IVE_IMAGE_TYPE_U8C1)
    {
...
HI_S32 IveImage2Mat(IVE_SRC_IMAGE_S *pstSrc, cv::Mat *dst)
{
    if(dst->cols != (HI_S32)pstSrc->u32Width || dst->rows != (HI_S32)pstSrc->u32Height)
    {
        printf("width or height didn't equal,line:%d\n",__LINE__);
        return -1;
    }
...
HI_S32 SAMPLE_IVE_YUV2BGR(IVE_SRC_IMAGE_S *pstInImg, IVE_DST_IMAGE_S *pstOutImg)
{
    HI_S32 s32Ret;
    ...
    s32Ret = HI_MPI_IVE_CSC(&IveHandle, pstInImg, pstOutImg, &stCtrl, HI_FALSE);
...
HI_S32 SAMPLE_IVE_BGR2YUV(IVE_SRC_IMAGE_S *pstInImg, IVE_DST_IMAGE_S *pstOutImg)
{
    HI_S32 s32Ret;
    IVE_HANDLE IveHandle;
    IVE_CSC_CTRL_S stCtrl;
...
HI_S32 SAMPLE_IVE_Or(IVE_SRC_IMAGE_S *pstSrcImg1, IVE_SRC_IMAGE_S *pstSrcImg2, IVE_DST_IMAGE_S *pstDstImg)
{
    HI_S32 s32Ret;
    IVE_HANDLE IveHandle;
...
Others: 
*************************************************/
HI_S32 SAMPLE_IVE_Sub(IVE_SRC_IMAGE_S *pstCurImg, IVE_SRC_IMAGE_S *pstLastImg, IVE_DST_IMAGE_S *pstDiffImg)
{
    HI_S32 s32Ret;
    IVE_HANDLE IveHandle;    
...
/*************************************************
Function:    SAMPLE_IVE_Add
Description: create two gray image add task, pstDstImg = (1 * pstSrcImg1) + (0.01 * pstSrcImg2)
Input:       pstSrcImg1:  type(U8C1) size(64x64~1920x1080)
             pstSrcImg2:  type(U8C1) size(64x64~1920x1080)
Output:      pstDstImg:   type(U8C1) size(64x64~1920x1080)
Return:      s32Ret
Others:
*************************************************/
HI_S32 SAMPLE_IVE_Add(int x, IVE_SRC_IMAGE_S *pstSrcImg1, int y, IVE_SRC_IMAGE_S *pstSrcImg2, IVE_DST_IMAGE_S *pstDstImg)
{
    HI_S32 s32Ret;
    IVE_HANDLE IveHandle;
    ...
    s32Ret = HI_MPI_IVE_Add(&IveHandle, pstSrcImg1, pstSrcImg2, pstDstImg, &stAddCtrl, HI_FALSE);
    SAMPLE_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,HI_FALSE,"Error(%#x),HI_MPI_IVE_Add failed!\n",s32Ret);
...
Others: 
*************************************************/
HI_S32 SAMPLE_IVE_Thresh(IVE_SRC_IMAGE_S *pstInImg, IVE_DST_IMAGE_S *pstOutImg, HI_U8 thr)
{
    HI_S32 s32Ret;
    IVE_HANDLE IveHandle;
...
/*************************************************
Function:    SAMPLE_IVE_Dilate_3x3
Description: create a binary image 3x3 template dilate tasks
...
*************************************************/
HI_S32 SAMPLE_IVE_Dilate_3x3(IVE_SRC_IMAGE_S *pstInImg, IVE_DST_IMAGE_S *pstOutImg)
{
    HI_S32 s32Ret;
    IVE_HANDLE IveHandle;
    ...
    pstCtrl.enMode  = IVE_ORD_STAT_FILTER_MODE_MAX;

    s32Ret = HI_MPI_IVE_OrdStatFilter(&IveHandle, pstInImg, pstOutImg, &pstCtrl, HI_FALSE);
    SAMPLE_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,"Error(%#x), HI_MPI_IVE_OrdStatFilter failed!\n",s32Ret);
    return s32Ret;
}


/*************************************************
Function:    SAMPLE_IVE_Erode_3x3
Description: create a binary image 3x3 template erode tasks
Input:       pstInImg:  type(U8C1) size(64x64~1920x1024)
Output:      pstOutImg: type(U8C1) size(64x64~1920x1024)
Return:      s32Ret
Others:      template coefficient can only to 0 or 255
*************************************************/
HI_S32 SAMPLE_IVE_Erode_3x3(IVE_SRC_IMAGE_S *pstInImg, IVE_DST_IMAGE_S *pstOutImg)
{
    HI_S32 s32Ret;
    IVE_HANDLE IveHandle;
    IVE_ORD_STAT_FILTER_CTRL_S pstCtrl;
    ...
    memset(&pstCtrl, 0, sizeof(pstCtrl));
    pstCtrl.enMode  = IVE_ORD_STAT_FILTER_MODE_MIN;
...
Others:
*************************************************/
HI_S32 SAMPLE_IVE_And(IVE_IMAGE_S *stImg1, IVE_IMAGE_S *stImg2, IVE_DST_IMAGE_S *stDst)
{
    HI_S32 s32Ret;
    IVE_HANDLE IveHandle;
...
/*************************************************
Function:    SAMPLE_IVE_CopyGray
Description: copy image
Input:       pstSrc:  type(U8C1) size(32x1~1920x1080)
Output:      pstDst:  type(U8C1) size(32x1~1920x1080)
Return:      s32Ret
Others:
*************************************************/
HI_S32 SAMPLE_IVE_CopyGray(IVE_SRC_IMAGE_S *pstSrc, IVE_DST_IMAGE_S *pstDst)
{
    ...
    s32Ret = HI_MPI_IVE_DMA(&hIveHandle, &stDataSrc, &stDataDst,&stCtrl,HI_FALSE);
    SAMPLE_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,"Error(%#x),HI_MPI_IVE_DMA failed!\n",s32Ret);

    return s32Ret;
}

HI_S32 SAMPLE_IVE_Copy420SP(IVE_SRC_IMAGE_S *pstSrc, IVE_DST_IMAGE_S *pstDst)
{
    HI_S32 s32Ret = HI_FAILURE;
    IVE_HANDLE hIveHandle;
    IVE_SRC_DATA_S stData0Src, stData1Src;
    ...
    stData0Src.u32Stride     = pstSrc->au32Stride[0];

    stData0Dst.u64VirAddr    = pstDst->au64VirAddr[0];
    stData0Dst.u64PhyAddr    = pstDst->au64PhyAddr[0];
    stData0Dst.u32Width      = pstDst->u32Width;
    stData0Dst.u32Height     = pstDst->u32Height;
    stData0Dst.u32Stride     = pstDst->au32Stride[0];
...
/*************************************************
Function:    SAMPLE_IVE_CountNonZero
Description: count non zero point
Input:       pstSrc:  type(U8C1) size(32x1~1920x1080)
             pstSfd:
Output:
Return:      s32Ret
